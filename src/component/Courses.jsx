...
function Courses(props) {
    ...
    const [studentFilteredCourses, setStudentFilteredCourses] = useState(props.subcourses);
    ...
    const studentFilters = (query) => {
        setStudentActiveFilter(query);
        if (query === 'all') {
            setStudentFilteredCourses(props.subcourses);
        } else {
            const filtered = props.subcourses.filter((course) =>
                course.name.toLowerCase().includes(query.toLowerCase())
            );
            setStudentFilteredCourses(filtered);
        }
    };
  ...
  return (
      <div className='flex-col h-auto py-10 w-full text-[rgb(227,227,227)] bg-white text-center justify-center text-4xl '>
        ...
        <div className='flex flex-wrap gap-4 mt-6 justify-center'>
            <button 
                className={`border-2 px-6 py-2 rounded-full text-lg cursor-pointer ${studentActiveFilter === 'all' ? 'bg-purple-500 text-white' : 'text-black border-gray-300'}`}
                onClick={() => studentFilters('all')}
            >
                All
            </button>
            {courses.map((course) => (
                <button 
                    className={`border-2 px-6 py-2 rounded-full text-lg cursor-pointer hover:bg-purple-500 hover:text-white ${studentActiveFilter === course ? 'bg-purple-500 text-white' : 'text-black border-gray-300'}`}
                    key={course} 
                    onClick={() => studentFilters(course)}
                >
                    {course}
                </button>
            ))}
            ...
            <div className='flex p-4 gap-6 mt-10 w-full bg-white overflow-x-auto no-scrollbar'>
            {studentFilteredCourses.map((course, index) => (
                <div className='gap-2 border border-gray-200 h-auto min-w-[300px] justify-center items-center flex flex-col rounded-lg text-black cursor-pointer list-none text-sm font-semibold text-center shadow-sm' key={index}>
                    <img src={course.img} alt={course.name} className='w-full h-48 object-cover rounded-t-lg' />
                    <li className='p-4'>{course.name}</li>
